...
class ShaderCinema {
  ...
  _map(src, src_min, src_max, dst_min, dst_max) {
    return (
      ((src - src_min) / (src_max - src_min)) * (dst_max - dst_min) + dst_min
    );
  }
  ...
  _setup_listeners() {
    ...
    this.container.addEventListener(
      ...
      function (event) {
        ...
        let ds = (event.deltaY / window.innerHeight) * this.viewport.size;
        let dmy =
          (0.5 * window.innerHeight - event.clientY) / window.innerHeight;
        let dmx =
          (-0.5 * window.innerWidth + event.clientX) / window.innerWidth;
